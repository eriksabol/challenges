...
void setup() {
  ...
  for (int i = 10000; i > 0; i--) {
    ...
    int currentNumber = i;
    int[] pinkColor = generateRandomTone(235, 0, 132);
    strokeWeight(thickness);
    stroke(pinkColor[0], pinkColor[1], pinkColor[2]);
...
int[] generateRandomTone(int red, int green, int blue) {
  ...
  float myRandomLuminousity=random(0.15, 0.90);
  float[] customHslColor = rgbToHsl(red, green, blue);
  customHslColor[2] = myRandomLuminousity;
  int[] customRGBtone = hslToRgb(customHslColor[0], customHslColor[1], customHslColor[2]);
